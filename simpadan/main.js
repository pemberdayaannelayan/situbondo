...
// PERBAIKAN BARU: 
// 4. FITUR PENGATURAN SISTEM DENGAN KODE KEAMANAN INPUT DAN DATA
// =====================================================
...
let appSettings = {
    ...
    // FITUR BARU: Pengaturan keamanan sistem
    securityCodeInput: '666666',
    securityInputEnabled: true,
    securityCodeDataMenu: '999999',
    securityDataMenuEnabled: true
};
...
// Modal untuk keamanan input data
let securityInputModal = null;
let securityDataMenuModal = null;
...
// --- FITUR BARU: PENGATURAN SISTEM KEAMANAN ---

// Fungsi untuk menampilkan modal keamanan input data
function showSecurityInputModal() {
    const modalHtml = `
    <div class="modal fade" id="securityInputModal" tabindex="-1" aria-labelledby="securityInputModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title" id="securityInputModalLabel">
                ...
                <div class="modal-body">
                    <div class="alert alert-info">
                        <i class="fas fa-info-circle me-2"></i>
                        Fitur ini melindungi akses ke menu Input Data dengan kode keamanan.
                    </div>
                    
                    <div class="security-status mb-4">
                        <div class="d-flex justify-content-between align-items-center">
                            <h6>Status Fitur Keamanan:</h6>
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="securityInputToggle" 
                                    ${appSettings.securityInputEnabled ? 'checked' : ''} 
                                    style="width: 3em; height: 1.5em;">
                                <label class="form-check-label ms-2" for="securityInputToggle">
                                    <span class="badge ${appSettings.securityInputEnabled ? 'bg-success' : 'bg-danger'}">
                                        ${appSettings.securityInputEnabled ? 'AKTIF' : 'NONAKTIF'}
                                    </span>
                                </label>
                            </div>
                        </div>
                    ...
                    <form id="securityInputForm">
                        <div class="mb-3">
                            <label for="currentInputCode" class="form-label">Kode Keamanan Saat Ini</label>
                            <div class="input-group">
                                <input type="password" class="form-control" id="currentInputCode" 
                                    placeholder="Masukkan kode saat ini">
                                <button class="btn btn-outline-secondary" type="button" id="toggleCurrentInputCode">
                        ...
                        <div class="mb-3">
                            <label for="newInputCode" class="form-label">Kode Keamanan Baru</label>
                            <div class="input-group">
                                <input type="password" class="form-control" id="newInputCode" 
                                    placeholder="Masukkan kode baru (minimal 6 digit)">
                                <button class="btn btn-outline-secondary" type="button" id="toggleNewInputCode">
                                    <i class="fas fa-eye"></i>
                                </button>
                            </div>
                            <div class="form-text">Kode baru minimal 6 digit angka atau huruf</div>
                        </div>
                        
                        <div class="mb-3">
                            <label for="confirmInputCode" class="form-label">Konfirmasi Kode Baru</label>
                            <div class="input-group">
                                <input type="password" class="form-control" id="confirmInputCode" 
                                    placeholder="Konfirmasi kode baru">
                                <button class="btn btn-outline-secondary" type="button" id="toggleConfirmInputCode">
                                    <i class="fas fa-eye"></i>
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="alert alert-warning">
                            ...
                            <strong>Catatan:</strong> Kode keamanan default adalah <code>666666</code> 
                            (jangan tampilkan ke publik)
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                    <button type="button" class="btn btn-primary" id="saveSecurityInputBtn">
    ...
    const existingModal = document.getElementById('securityInputModal');
    if (existingModal) {
    ...
    // Inisialisasi modal
    securityInputModal = new bootstrap.Modal(document.getElementById('securityInputModal'));
    
    // Setup event listeners
    document.getElementById('securityInputToggle').addEventListener('change', function() {
    ...
    document.getElementById('toggleCurrentInputCode').addEventListener('click', function() {
        const input = document.getElementById('currentInputCode');
        const icon = this.querySelector('i');
        togglePasswordVisibility(input, icon);
    });
    
    document.getElementById('toggleNewInputCode').addEventListener('click', function() {
        const input = document.getElementById('newInputCode');
        const icon = this.querySelector('i');
        togglePasswordVisibility(input, icon);
    });
    
    document.getElementById('toggleConfirmInputCode').addEventListener('click', function() {
        const input = document.getElementById('confirmInputCode');
        const icon = this.querySelector('i');
        togglePasswordVisibility(input, icon);
    });
    
    // Setup save button
    document.getElementById('saveSecurityInputBtn').addEventListener('click', function() {
        saveSecurityInputSettings();
    });
    
    // Tampilkan modal
    securityInputModal.show();
}

// Fungsi untuk menampilkan modal keamanan menu data nelayan
function showSecurityDataMenuModal() {
    const modalHtml = `
    <div class="modal fade" id="securityDataMenuModal" tabindex="-1" aria-labelledby="securityDataMenuModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header bg-success text-white">
                    <h5 class="modal-title" id="securityDataMenuModalLabel">
                        <i class="fas fa-database me-2"></i>Keamanan Menu Data Nelayan
                    </h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="alert alert-info">
                        <i class="fas fa-info-circle me-2"></i>
                        Fitur ini melindungi akses ke menu Data Nelayan dengan kode keamanan.
                    </div>
                    ...
                    <div class="security-status mb-4">
                        <div class="d-flex justify-content-between align-items-center">
                            <h6>Status Fitur Keamanan:</h6>
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="securityDataMenuToggle" 
                                    ${appSettings.securityDataMenuEnabled ? 'checked' : ''} 
                                    style="width: 3em; height: 1.5em;">
                                <label class="form-check-label ms-2" for="securityDataMenuToggle">
                                    <span class="badge ${appSettings.securityDataMenuEnabled ? 'bg-success' : 'bg-danger'}">
                                        ${appSettings.securityDataMenuEnabled ? 'AKTIF' : 'NONAKTIF'}
                                    </span>
                                </label>
                            </div>
                        </div>
                    ...
                    <form id="securityDataMenuForm">
                        <div class="mb-3">
                            <label for="currentDataMenuCode" class="form-label">Kode Keamanan Saat Ini</label>
                            <div class="input-group">
                                <input type="password" class="form-control" id="currentDataMenuCode" 
                                    placeholder="Masukkan kode saat ini">
                                <button class="btn btn-outline-secondary" type="button" id="toggleCurrentDataMenuCode">
                                    <i class="fas fa-eye"></i>
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            <label for="newDataMenuCode" class="form-label">Kode Keamanan Baru</label>
                            <div class="input-group">
                                <input type="password" class="form-control" id="newDataMenuCode" 
                                    placeholder="Masukkan kode baru (minimal 6 digit)">
                                <button class="btn btn-outline-secondary" type="button" id="toggleNewDataMenuCode">
                                    <i class="fas fa-eye"></i>
                                </button>
                            </div>
                            <div class="form-text">Kode baru minimal 6 digit angka atau huruf</div>
                        ...
                        <div class="mb-3">
                            <label for="confirmDataMenuCode" class="form-label">Konfirmasi Kode Baru</label>
                            <div class="input-group">
                                <input type="password" class="form-control" id="confirmDataMenuCode" 
                                    placeholder="Konfirmasi kode baru">
                                <button class="btn btn-outline-secondary" type="button" id="toggleConfirmDataMenuCode">
                                    <i class="fas fa-eye"></i>
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="alert alert-warning">
                            <i class="fas fa-exclamation-triangle me-2"></i>
                            <strong>Catatan:</strong> Kode keamanan default adalah <code>999999</code> 
                            (jangan tampilkan ke publik)
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                    <button type="button" class="btn btn-success" id="saveSecurityDataMenuBtn">
                        <i class="fas fa-save me-2"></i>Simpan Perubahan
                    </button>
                </div>
            </div>
    ...
    // Hapus modal sebelumnya jika ada
    const existingModal = document.getElementById('securityDataMenuModal');
    if (existingModal) {
        existingModal.remove();
    }
    ...
    // Inisialisasi modal
    securityDataMenuModal = new bootstrap.Modal(document.getElementById('securityDataMenuModal'));
    
    // Setup event listeners
    document.getElementById('securityDataMenuToggle').addEventListener('change', function() {
        const statusBadge = this.parentElement.querySelector('.badge');
        if (this.checked) {
            statusBadge.className = 'badge bg-success';
            statusBadge.textContent = 'AKTIF';
    ...
    // Setup toggle password visibility
    document.getElementById('toggleCurrentDataMenuCode').addEventListener('click', function() {
        const input = document.getElementById('currentDataMenuCode');
        const icon = this.querySelector('i');
        togglePasswordVisibility(input, icon);
    });
    
    document.getElementById('toggleNewDataMenuCode').addEventListener('click', function() {
        const input = document.getElementById('newDataMenuCode');
        const icon = this.querySelector('i');
        togglePasswordVisibility(input, icon);
    });
    
    document.getElementById('toggleConfirmDataMenuCode').addEventListener('click', function() {
        const input = document.getElementById('confirmDataMenuCode');
        const icon = this.querySelector('i');
        togglePasswordVisibility(input, icon);
    });
    
    // Setup save button
    document.getElementById('saveSecurityDataMenuBtn').addEventListener('click', function() {
        saveSecurityDataMenuSettings();
    });
    
    // Tampilkan modal
    securityDataMenuModal.show();
}
...
// Fungsi untuk menyimpan pengaturan keamanan input data
function saveSecurityInputSettings() {
    const currentCode = document.getElementById('currentInputCode').value;
    const newCode = document.getElementById('newInputCode').value;
    const confirmCode = document.getElementById('confirmInputCode').value;
    const isEnabled = document.getElementById('securityInputToggle').checked;
    
    // Validasi kode saat ini
    if (currentCode !== appSettings.securityCodeInput) {
        showNotification('Kode keamanan saat ini salah!', 'error');
        return;
    }
    
    // Validasi jika mengubah kode
    if (newCode || confirmCode) {
        ...
        // Simpan kode baru
        appSettings.securityCodeInput = newCode;
    }
    
    // Simpan status enable/disable
    appSettings.securityInputEnabled = isEnabled;
    
    // Simpan ke localStorage
    saveSettings();
    
    // Tutup modal
    securityInputModal.hide();
    
    // Clear form
    document.getElementById('currentInputCode').value = '';
    document.getElementById('newInputCode').value = '';
    document.getElementById('confirmInputCode').value = '';
    
    showNotification('Pengaturan keamanan input data berhasil disimpan!', 'success');
}

// Fungsi untuk menyimpan pengaturan keamanan menu data nelayan
function saveSecurityDataMenuSettings() {
    const currentCode = document.getElementById('currentDataMenuCode').value;
    const newCode = document.getElementById('newDataMenuCode').value;
    const confirmCode = document.getElementById('confirmDataMenuCode').value;
    const isEnabled = document.getElementById('securityDataMenuToggle').checked;
    
    // Validasi kode saat ini
    if (currentCode !== appSettings.securityCodeDataMenu) {
        showNotification('Kode keamanan saat ini salah!', 'error');
        return;
    }
    ...
    if (newCode || confirmCode) {
        ...
        // Simpan kode baru
        appSettings.securityCodeDataMenu = newCode;
    }
    
    // Simpan status enable/disable
    appSettings.securityDataMenuEnabled = isEnabled;
    
    // Simpan ke localStorage
    saveSettings();
    
    // Tutup modal
    securityDataMenuModal.hide();
    
    // Clear form
    document.getElementById('currentDataMenuCode').value = '';
    document.getElementById('newDataMenuCode').value = '';
    document.getElementById('confirmDataMenuCode').value = '';
    
    showNotification('Pengaturan keamanan menu data nelayan berhasil disimpan!', 'success');
}

// Fungsi untuk memverifikasi akses ke menu input data
function checkInputDataAccess() {
    if (!appSettings.securityInputEnabled) {
        return true; // Fitur nonaktif, langsung izinkan akses
    }
    
    const userCode = prompt("Masukkan kode keamanan untuk mengakses menu Input Data:");
    if (userCode === appSettings.securityCodeInput) {
        return true;
...
// Fungsi untuk memverifikasi akses ke menu data nelayan
function checkDataMenuAccess() {
    if (!appSettings.securityDataMenuEnabled) {
        return true; // Fitur nonaktif, langsung izinkan akses
    }
    
    const userCode = prompt("Masukkan kode keamanan untuk mengakses menu Data Nelayan:");
    if (userCode === appSettings.securityCodeDataMenu) {
        return true;
    } else if (userCode !== null) {
        alert("Kode keamanan salah!");
        return false;
...
function setupEventListeners() {
    ...
    // FITUR BARU: Tombol untuk pengaturan keamanan input data
    const securityInputBtn = document.getElementById('btnSecurityInput');
    if (securityInputBtn) {
        securityInputBtn.addEventListener('click', showSecurityInputModal);
    }
    
    // FITUR BARU: Tombol untuk pengaturan keamanan menu data nelayan
    const securityDataMenuBtn = document.getElementById('btnSecurityDataMenu');
    if (securityDataMenuBtn) {
        securityDataMenuBtn.addEventListener('click', showSecurityDataMenuModal);
    }
    
    // FITUR BARU: Modifikasi akses ke menu input data
    const inputTab = document.getElementById('v-pills-input-tab');
    if (inputTab) {
        inputTab.addEventListener('click', function(e) {
            if (!checkInputDataAccess()) {
                e.preventDefault();
                e.stopPropagation();
                // Alihkan ke tab dashboard
                document.getElementById('v-pills-dashboard-tab').click();
            }
        });
    }
    
    // FITUR BARU: Modifikasi akses ke menu data nelayan
    const dataTab = document.getElementById('v-pills-data-tab');
    if (dataTab) {
        dataTab.addEventListener('click', function(e) {
            if (!checkDataMenuAccess()) {
                e.preventDefault();
                e.stopPropagation();
                // Alihkan ke tab dashboard
                document.getElementById('v-pills-dashboard-tab').click();
...
function loadSettings() { 
    ...
    if(s) {
        try {
            ...
            // FITUR BARU: Load pengaturan keamanan jika ada
            if (!loadedSettings.securityCodeInput) {
                loadedSettings.securityCodeInput = '666666';
            }
            if (!loadedSettings.securityInputEnabled) {
                loadedSettings.securityInputEnabled = true;
            }
            if (!loadedSettings.securityCodeDataMenu) {
                loadedSettings.securityCodeDataMenu = '999999';
            }
            if (!loadedSettings.securityDataMenuEnabled) {
                loadedSettings.securityDataMenuEnabled = true;
            }
...
// FITUR BARU: Ekspos fungsi pengaturan sistem
window.showSecurityInputModal = showSecurityInputModal;
window.showSecurityDataMenuModal = showSecurityDataMenuModal;
window.checkInputDataAccess = checkInputDataAccess;
window.checkDataMenuAccess = checkDataMenuAccess;
