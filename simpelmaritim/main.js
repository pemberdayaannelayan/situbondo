...
// Data insight (Pengetahuan & Wawasan)
const insightData = [
    {
        icon: "fas fa-sun",
...
function safeGetElement(id) {
    ...
    if (!el) console.warn(`Elemen dengan id "${id}" tidak ditemukan.`);
    return el;
}

// ==================== RENDER INSIGHT ====================
function renderInsight() {
    const grid = safeGetElement('insightGrid');
    if (!grid) return;
    let html = '';
...
function updateDateTime() {
    ...
    const currentTimeEl = safeGetElement('currentTime');
    if (currentDateEl) currentDateEl.textContent = now.toLocaleDateString('id-ID', { weekday:'long', year:'numeric', month:'long', day:'numeric' });
...
async function fetchOverlayWeather() {
    try {
        ...
        if (data.current_weather) {
            ...
            const tempEl = safeGetElement('overlayTemp');
            const descEl = safeGetElement('overlayDesc');
            if (tempEl) tempEl.innerHTML = w.temperature + '°C';
            ...
            if (descEl) descEl.textContent = codes[w.weathercode] || 'Kondisi khusus';
        } else throw new Error('Data tidak lengkap');
    } catch (error) {
        console.warn('Gagal memuat cuaca overlay:', error);
        const tempEl = safeGetElement('overlayTemp');
        const descEl = safeGetElement('overlayDesc');
        if (tempEl) tempEl.innerHTML = '--°C';
        if (descEl) descEl.textContent = 'Situbondo';
    }
}

function updateOverlayTime() {
    const now = new Date();
    const timeEl = safeGetElement('overlayTime');
    const dateEl = safeGetElement('overlayDate');
...
function startApp() {
    stopParticles();
    const loginOverlay = safeGetElement('loginOverlay');
    const mainContent = safeGetElement('mainContent');
    const audioContainer = safeGetElement('audioPlayerContainer');
...
document.addEventListener('DOMContentLoaded', function() {
    const nantiBtn = safeGetElement('nantiSajaBtn');
    const putarBtn = safeGetElement('putarAudioBtn');
    if (nantiBtn) nantiBtn.addEventListener('click', startApp);
    if (putarBtn) putarBtn.addEventListener('click', function() {
        const audio = document.getElementById('loginAudio');
        if (audio) {
            document.body.appendChild(audio); // pindahkan agar tetap jalan
...
// Fungsi global untuk tombol di popup
window.setAsStart = function(lat, lng) {
    const startLat = safeGetElement('startLat');
    const startLon = safeGetElement('startLon');
    if (startLat) startLat.value = lat.toFixed(5);
    if (startLon) startLon.value = lng.toFixed(5);
    alert('Titik awal telah diisi.');
};
window.setAsEnd = function(lat, lng) {
    const endLat = safeGetElement('endLat');
    const endLon = safeGetElement('endLon');
    if (endLat) endLat.value = lat.toFixed(5);
    if (endLon) endLon.value = lng.toFixed(5);
...
document.addEventListener('DOMContentLoaded', function() {
    const clearBtn = safeGetElement('clearMarkersBtn');
    if (clearBtn) {
    ...
    const routeBtn = safeGetElement('routeBtn');
    if (routeBtn) {
        routeBtn.addEventListener('click', function() {
            const startLat = parseFloat(safeGetElement('startLat')?.value);
            const startLon = parseFloat(safeGetElement('startLon')?.value);
            const endLat = parseFloat(safeGetElement('endLat')?.value);
            const endLon = parseFloat(safeGetElement('endLon')?.value);
    ...
    const myLocBtn = safeGetElement('myLocationBtn');
    if (myLocBtn) {
    ...
    const setStartBtn = safeGetElement('setStartLocationBtn');
    if (setStartBtn) {
        setStartBtn.addEventListener('click', function() {
            getCurrentPosition((lat, lon) => {
                ...
                const startLon = safeGetElement('startLon');
                if (startLat) startLat.value = lat.toFixed(5);
                if (startLon) startLon.value = lon.toFixed(5);
                alert('Koordinat awal diisi.');
            });
        });
    }

    const setEndBtn = safeGetElement('setEndLocationBtn');
    if (setEndBtn) {
        setEndBtn.addEventListener('click', function() {
            getCurrentPosition((lat, lon) => {
                ...
                const endLon = safeGetElement('endLon');
                if (endLat) endLat.value = lat.toFixed(5);
                if (endLon) endLon.value = lon.toFixed(5);
                alert('Koordinat tujuan diisi.');
            });
        });
    }

    const checkRouteBtn = safeGetElement('checkRouteToDestinationBtn');
    if (checkRouteBtn) {
        checkRouteBtn.addEventListener('click', function() {
            getCurrentPosition((lat, lon) => {
                const startLat = safeGetElement('startLat');
                const startLon = safeGetElement('startLon');
                if (startLat) startLat.value = lat.toFixed(5);
                ...
                if (isNaN(endLat) || isNaN(endLon)) {
                    alert('Titik tujuan belum diisi.');
                    return;
                }
...
function showRoute(startLat, startLon, endLat, endLon) {
    ...
    routingControl.on('routesfound', function(e) {
        ...
        const routeInfo = safeGetElement('routeInfo');
        if (routeInfo) {
            routeInfo.innerHTML = `<i class="fas fa-info-circle"></i> Jarak: ${distance.toFixed(2)} km | Estimasi: ${timeStr}`;
    ...
    routingControl.on('routingerror', function() {
        const routeInfo = safeGetElement('routeInfo');
        if (routeInfo) {
            routeInfo.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Gagal menghitung rute.';
            routeInfo.classList.add('show');
        }
    });
}
...
function setLoadingState(isLoading) {
    ...
    if (refreshIcon) {
        refreshIcon.className = isLoading ? 'fas fa-spinner fa-pulse' : 'fas fa-sync-alt';
    }
}
...
async function fetchSeaTemperature() {
    try {
        ...
        while (idx >= 0 && temps[idx] === null) idx--;
        if (idx >= 0) {
            const valueEl = safeGetElement('suhuValueCard');
            const timeEl = safeGetElement('suhuTimeCard');
            if (valueEl) valueEl.innerHTML = temps[idx].toFixed(1) + ' <span class="unit">°C</span>';
            if (timeEl) timeEl.innerHTML = '<i class="fas fa-clock"></i> ' + new Date(times[idx] + 'Z').toLocaleString('id-ID', { dateStyle: 'medium', timeStyle: 'short' });
        } else throw new Error();
    } catch {
        const valueEl = safeGetElement('suhuValueCard');
        const timeEl = safeGetElement('suhuTimeCard');
        if (valueEl) valueEl.innerHTML = '-- <span class="unit">°C</span>';
        if (timeEl) timeEl.innerHTML = '<i class="fas fa-clock"></i> Gagal memuat';
    }
}
...
async function fetchWaveData() {
    try {
        ...
        while (idx >= 0 && heights[idx] === null) idx--;
        if (idx >= 0) {
            const waveVal = safeGetElement('waveValueCard');
            const waveTime = safeGetElement('waveTimeCard');
            const waveDir = safeGetElement('waveDirection');
            const wavePer = safeGetElement('wavePeriod');
            if (waveVal) waveVal.innerHTML = (heights[idx] ? heights[idx].toFixed(1) : '--') + ' <span class="unit">m</span>';
            if (waveTime) waveTime.innerHTML = '<i class="fas fa-clock"></i> ' + new Date(times[idx] + 'Z').toLocaleString('id-ID', { dateStyle: 'medium', timeStyle: 'short' });
            if (waveDir) waveDir.textContent = dirs?.[idx]?.toFixed(0) || '--';
            if (wavePer) wavePer.textContent = pers?.[idx]?.toFixed(1) || '--';
        } else throw new Error();
    } catch {
        const waveVal = safeGetElement('waveValueCard');
        const waveTime = safeGetElement('waveTimeCard');
        const waveDir = safeGetElement('waveDirection');
        const wavePer = safeGetElement('wavePeriod');
        if (waveVal) waveVal.innerHTML = '-- <span class="unit">m</span>';
        if (waveTime) waveTime.innerHTML = '<i class="fas fa-clock"></i> Gagal';
        if (waveDir) waveDir.textContent = '--';
        if (wavePer) wavePer.textContent = '--';
    }
}
...
async function fetchWeatherData() {
    try {
        ...
        if (data.current_weather) {
            const w = data.current_weather;
            const tempCard = safeGetElement('weatherTempCard');
            const timeCard = safeGetElement('weatherTimeCard');
            const windSpeed = safeGetElement('windSpeed');
            const weatherDesc = safeGetElement('weatherDesc');
            if (tempCard) tempCard.innerHTML = w.temperature + ' <span class="unit">°C</span>';
            if (timeCard) timeCard.innerHTML = '<i class="fas fa-clock"></i> ' + new Date(w.time + 'Z').toLocaleString('id-ID', { dateStyle: 'medium', timeStyle: 'short' });
            if (windSpeed) windSpeed.textContent = w.windspeed;
            const codes = {0:'Cerah',1:'Cerah berawan',2:'Berawan',3:'Berawan tebal',45:'Kabut',48:'Kabut beku',51:'Gerimis ringan',53:'Gerimis',55:'Gerimis deras',56:'Gerimis beku',57:'Gerimis beku deras',61:'Hujan ringan',63:'Hujan',65:'Hujan deras',66:'Hujan beku',67:'Hujan beku deras',71:'Salju ringan',73:'Salju',75:'Salju deras',77:'Butiran salju',80:'Hujan lokal',81:'Hujan lokal sedang',82:'Hujan lokal deras',85:'Salju ringan',86:'Salju deras',95:'Badai petir',96:'Badai petir hujan es',99:'Badai petir hujan es deras'};
            if (weatherDesc) weatherDesc.textContent = codes[w.weathercode] || 'Kondisi khusus';
        } else throw new Error();
    } catch {
        const tempCard = safeGetElement('weatherTempCard');
        const timeCard = safeGetElement('weatherTimeCard');
        const windSpeed = safeGetElement('windSpeed');
        const weatherDesc = safeGetElement('weatherDesc');
        if (tempCard) tempCard.innerHTML = '-- <span class="unit">°C</span>';
        if (timeCard) timeCard.innerHTML = '<i class="fas fa-clock"></i> Gagal';
...
function calculateMoonPhase() {
    ...
    const moonPhaseEl = safeGetElement('moonPhase');
    const moonEmojiEl = safeGetElement('moonEmoji');
    if (moonPhaseEl) moonPhaseEl.textContent = name;
    if (moonEmojiEl) moonEmojiEl.textContent = emoji;
}
...
function renderAgenda() {
    const container = safeGetElement('agendaItems');
    const noAgenda = safeGetElement('noAgenda');
    if (!container || !noAgenda) return;
...
window.editAgenda = function(index) {
    ...
    const titleEl = safeGetElement('agendaTitle');
    const dateTimeEl = safeGetElement('agendaDateTime');
    const descEl = safeGetElement('agendaDesc');
    const formTitle = safeGetElement('agendaFormTitle');
    const addBtn = safeGetElement('addAgendaBtn');
    const cancelBtn = safeGetElement('cancelEditBtn');
    if (titleEl) titleEl.value = item.title;
    if (dateTimeEl) dateTimeEl.value = item.dateTime.slice(0, 16);
    if (descEl) descEl.value = item.desc || '';
    if (formTitle) formTitle.innerText = 'Edit Agenda';
    if (addBtn) addBtn.innerHTML = '<i class="fas fa-save"></i> Update';
...
window.cancelEdit = function() {
    ...
    if (formTitle) formTitle.innerText = 'Tambah Agenda';
    if (addBtn) addBtn.innerHTML = '<i class="fas fa-plus-circle"></i> Simpan';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (addBtn) {
        addBtn.addEventListener('click', function() {
            const title = safeGetElement('agendaTitle')?.value.trim();
            const dateTime = safeGetElement('agendaDateTime')?.value;
            const desc = safeGetElement('agendaDesc')?.value.trim();
            if (!title || !dateTime) {
...
function showFloatingNotification() {
    const notif = safeGetElement('floatingNotif');
    if (notif && localStorage.getItem('floatingNotifDismissed') !== 'true') {
        notif.style.display = 'block';
    }
}

document.addEventListener('DOMContentLoaded', function() {
    const notifTidak = safeGetElement('notifTidak');
    const notifIya = safeGetElement('notifIya');
    if (notifTidak) {
        notifTidak.addEventListener('click', function() {
            const notif = safeGetElement('floatingNotif');
    ...
    if (notifIya) {
        notifIya.addEventListener('click', function() {
...
window.closeAudioPlayer = function() {
    const container = safeGetElement('audioPlayerContainer');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // mainContent sudah display none dari HTML
    initParticles();
    renderAgenda();
    ...
    setInterval(fetchOverlayWeather, 600000); // setiap 10 menit
    setInterval(fetchAllData, 1800000); // setiap 30 menit
});
