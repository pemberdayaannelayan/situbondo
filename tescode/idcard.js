// ================================
// IDCARD.JS - SIMATA ID CARD GENERATOR
// Version: 10.1 - Layout Revisi
...
/**
 * Fungsi utama untuk menghasilkan ID Card nelayan - DIPERBAIKI TOTAL DENGAN LAYOUT BARU
 */
function generateSimataIDCard(nelayanData) {
    ...
    try {
        ...
        const qrData = "https://www.dinasperikanansitubondo.com/simata/#verifikasi";
...
/**
 * Generate PDF untuk ID Card dengan LAYOUT BARU (ID di kiri bawah, QR di kanan bawah)
 */
async function generateIDCardPDFWithNewLayout(nelayanData, qrContainer) {
    ...
    // Buat PDF dengan orientasi landscape untuk kartu ID (ukuran standar)
    const doc = new jsPDF({
    ...
    let profesiDisplay = nelayanData.profesi.toUpperCase();
    if (profesiDisplay.length > 25) {
        profesiDisplay = profesiDisplay.substring(0, 23) + '...';
    }
    doc.text(profesiDisplay, rightX + 17.5, currentY, { align: 'center' });
    ...
    // 5. DATA KAPAL (jika pemilik kapal)
    if (nelayanData.status === "Pemilik Kapal") {
        currentY += lineHeight;
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(6);
    ...
    // 7. TEKS "Kartu Identitas Resmi - Berlaku Selamanya" di bawah ID
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(5);
    doc.setTextColor(100, 100, 100);
    doc.text("Kartu Identitas Resmi - Berlaku Selamanya", 5, 47.5);
    
    // 8. QR CODE DI KANAN BAWAH
    if (qrContainer) {
...
/**
 * Fungsi untuk menghasilkan preview ID Card dalam format HTML dengan LAYOUT BARU
 */
function previewIDCardHTML(nelayanData) {
    ...
    // Cek apakah pemilik kapal untuk menampilkan info kapal
    const isPemilikKapal = nelayanData.status === "Pemilik Kapal";
    const kapalInfoHTML = isPemilikKapal ? `
        <div class="id-card-improved-detail">
            <span class="id-card-improved-label">NAMA KAPAL:</span>
            <span class="id-card-improved-value">${nelayanData.namaKapal || '-'}</span>
        </div>
        <div class="id-card-improved-detail">
            <span class="id-card-improved-label">JENIS KAPAL:</span>
            <span class="id-card-improved-value">${nelayanData.jenisKapal || '-'}</span>
        </div>
    ` : '';
                    ...
                    <div class="id-card-improved-details">
                        ...
                        <div class="id-card-improved-detail">
                            ...
                            <span class="id-card-improved-value">
                                <span class="id-card-improved-badge ${profesiClass}">${nelayanData.profesi.toUpperCase()}</span>
                            </span>
                        </div>
                        <div class="id-card-improved-detail">
...
function safeGenerateIDCard(id) {
    const appData = JSON.parse(localStorage.getItem('nelayanData') || '[]');
    const nelayanData = appData.find(item => item.id == id);
    ...
    if (!checkRequiredLibraries()) {
        if (typeof showNotification === 'function') {
            showNotification('Library PDF atau QRCode tidak tersedia. Pastikan koneksi internet stabil.', 'error');
        }
        return;
    }
...
function generateQRDataForIDCard(nelayanData) {
    return "https://www.dinasperikanansitubondo.com/simata/#verifikasi";
}
...
console.log('✅ SIMATA ID Card Generator v10.1 loaded successfully');
