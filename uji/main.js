...
// Deklarasi namespace global untuk aplikasi
const SIMATA = (function() {
    ...
    const FISH_CATEGORIES = {
        ...
        "Pelagis Besar": [
            ...
            {name: "Ikan Cucut (Carcharhinus limbatus)", latin: "Carcharhinus limbatus", desc: "Hiu permukaan dengan sirip black. Hidup di perairan pantai."}
        ]
    };
    ...
    function displayCurrentDate() {
        ...
        if (currentDateDisplay) {
            currentDateDisplay.innerHTML = `<i class="fas fa-calendar-day me-2"></i>${dateString}`;
        }
        const passwordHint = document.getElementById('passwordHint');
        if (passwordHint) {
            passwordHint.innerHTML = `Kode keamanan berubah setiap hari berdasarkan tanggal.`;
        }
    }
    ...
    function updateAlatTangkapByKapal() {
        ...
        if (!jenisKapal || !alatTangkapSelect) return;
        
        const jenisKapalValue = jenisKapal.value;
        if (!jenisKapalValue) {
            if (apiMappingInfo) apiMappingInfo.style.display = 'none';
            return;
        }
        
        const availableAPIs = KAPAL_API_MAPPING[jenisKapalValue] || Object.keys(API_INFO);
        ...
        if (apiMappingInfo) {
            apiMappingInfo.innerHTML = `<strong>${jenisKapalValue}</strong> biasanya digunakan untuk: ${availableAPIs.join(', ')}`;
    ...
    function displayVerifyResult(result) {
        ...
        if (!card || !header || !icon || !title || !subtitle || !content) return;
    ...
    function setVerifyExample(type) {
        ...
        if (type === 'kin') {
            ...
            input.focus();
            if (breadcrumbItems[0]) breadcrumbItems[0].classList.add('active');
        } else if (type === 'nik') {
            input.value = '';
            input.placeholder = 'Masukkan NIK 16 digit';
            input.focus();
            if (breadcrumbItems[1]) breadcrumbItems[1].classList.add('active');
        } else if (type === 'all') {
            if (breadcrumbItems[2]) breadcrumbItems[2].classList.add('active');
            showAllKIN();
        }
    }
    ...
    function showAllKIN() {
        ...
        // Update count
        if (totalKinCount) totalKinCount.textContent = dataWithKIN.length;
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="btn-group btn-group-sm">
                                <button class="btn btn-outline-primary" onclick="SIMATA.verifyKINAndShow('${d.kodeValidasi}')" title="Verifikasi">
                                    <i class="fas fa-check"></i>
                                </button>
                                <button class="btn btn-outline-info" onclick="SIMATA.viewDetail('${d.id}')" title="Detail">
                                    <i class="fas fa-eye"></i></button>
                                <button class="btn btn-outline-success" onclick="SIMATA.safeGenerateIDCard('${d.id}')" title="ID Card">
    ...
    function verifyKINAndShow(input) {
        ...
        if (verifyInput) verifyInput.value = input;
    }
    ...
    function showFishInfoModal() {
        if (fishInfoModal) fishInfoModal.show();
    }
    ...
    function generateTabelPdf() {
        ...
        const qrRight = document.getElementById('qr-right');
        if (qrRight) qrRight.innerHTML = "";
        ...
        setTimeout(() => {
            try {
                ...
                if (Object.keys(currentFilter).length > 0 || (searchData && searchData.value)) {
                    ...
                    if (searchData && searchData.value) filterParts.push(`Pencarian: "${searchData.value}"`);
...
window.SIMATA_BACKUP_ENCRYPTED = '${encryptedData}';`;
    ...
    function updatePagination(totalItems) {
        ...
        first.innerHTML = `<a class="page-link" href="#" onclick="SIMATA.goToPage(1); return false;" title="Halaman Pertama">&laquo;</a>`;
        ul.appendChild(first);
        
        // Tombol sebelumnya
        const prev = document.createElement('li');
        prev.className = `page-item ${currentPage === 1 ? 'disabled' : ''}`;
        prev.innerHTML = `<a class="page-link" href="#" onclick="SIMATA.goToPage(${currentPage-1}); return false;" title="Halaman Sebelumnya">&lt;</a>`;
        ul.appendChild(prev);
        ...
        for (let i = startPage; i <= endPage; i++) {
            ...
            li.innerHTML = `<a class="page-link" href="#" onclick="SIMATA.goToPage(${i}); return false;">${i}</a>`;
            ul.appendChild(li);
        }
        ...
        next.innerHTML = `<a class="page-link" href="#" onclick="SIMATA.goToPage(${currentPage+1}); return false;" title="Halaman Berikutnya">&gt;</a>`;
        ul.appendChild(next);
        
        // Tombol terakhir
        const last = document.createElement('li');
        last.className = `page-item ${currentPage === totalPages ? 'disabled' : ''}`;
        last.innerHTML = `<a class="page-link" href="#" onclick="SIMATA.goToPage(${totalPages}); return false;" title="Halaman Terakhir">&raquo;</a>`;
        ul.appendChild(last);
    }
    ...
    function renderDataTable() {
        ...
        if (totalItems === 0) {
            ...
            pageData.forEach((d, i) => {
                ...
                const row = `<tr class="${rowClass}">
                    <td class="text-center"><input type="checkbox" class="row-checkbox" value="${d.id}" onchange="SIMATA.toggleBulkDeleteBtn()"></td>
                    <td class="text-center">${start + i + 1}</td>
                    <td onclick="SIMATA.viewDetail('${d.id}')" class="clickable-name col-id-cell">
                    ...
                    <td class="col-action sticky-col-right">
                        <div class="btn-group shadow-sm" role="group">
                            <button class="btn btn-sm btn-info text-white" onclick="SIMATA.viewDetail('${d.id}')" title="Lihat Detail"><i class="fas fa-eye"></i></button>
                            <button class="btn btn-sm btn-warning text-white" onclick="SIMATA.editData('${d.id}')" title="Edit"><i class="fas fa-pencil-alt"></i></button>
                            <button class="btn btn-sm btn-idcard" onclick="SIMATA.safeGenerateIDCard('${d.id}')" title="Cetak ID Card"><i class="fas fa-id-card"></i></button>
                            <button class="btn btn-sm btn-danger" onclick="SIMATA.deleteData('${d.id}')" title="Hapus"><i class="fas fa-trash"></i></button>
                        </div>
                    </td>
                </tr>`;
                tableBody.innerHTML += row;
        ...
        if (tableInfo) tableInfo.textContent = `Menampilkan ${startItem}-${endItem} dari ${totalItems} data`;
    ...
    function applyFilter() {
        ...
        const filterProfesi = document.getElementById('filterProfesi');
    ...
    function setupInfoTooltips() {
        ...
        if (alatTangkapSelect && apiInfoDiv) {
        ...
        if (jenisKapalSelect && kapalInfoDiv) {
    ...
    function setupEventListeners() {
        ...
        if (loginForm) {
            loginForm.addEventListener('submit', function(e) {
                ...
                setTimeout(() => {
                    ...
                    if (loginSuccessModal) loginSuccessModal.show();
        ...
        if (verifyBtn) {
            verifyBtn.addEventListener('click', function() {
                ...
                const result = verifyKIN(input.value);
                displayVerifyResult(result);
            });
        }
        ...
        if (restoreFileInput) {
            restoreFileInput.addEventListener('change', function() {
                const restoreDataBtn = document.getElementById('restoreDataBtn');
                if (restoreDataBtn) restoreDataBtn.disabled = !this.files.length;
            });
        }
        ...
        if (officialForm) {
            officialForm.addEventListener('submit', function(e) {
                ...
                const officialNameValue = officialName.value;
                const officialNipValue = officialNip.value;
                const officialPositionValue = officialPosition.value;
                
                if (!officialNameValue || !officialNipValue || !officialPositionValue) {
                    showNotification('Harap isi semua field data pejabat!', 'error');
                    return;
                }
                
                appSettings.officialName = officialNameValue;
                appSettings.officialNip = officialNipValue;
                appSettings.officialPosition = officialPositionValue;
        ...
        if (sensorCodeForm) {
            sensorCodeForm.addEventListener('submit', function(e) {
                e.preventDefault();
                const currentCode = document.getElementById('currentSensorCode');
                const newCode = document.getElementById('newSensorCode');
                const confirmCode = document.getElementById('confirmSensorCode');
                
                if (!currentCode || !newCode || !confirmCode) return;
                
                const currentCodeValue = currentCode.value;
                const newCodeValue = newCode.value;
                const confirmCodeValue = confirmCode.value;
                
                if (currentCodeValue !== appSettings.securityCodeSensor) {
                    showNotification('Kode keamanan sensor saat ini salah!', 'error');
                    return;
                }
                
                if (newCodeValue.length < 6) {
                ...
                appSettings.securityCodeSensor = newCodeValue;
                saveSettings();
                currentCode.value = '';
                newCode.value = '';
                confirmCode.value = '';
    ...
    function handleFormSubmit(e) {
        ...
        const kodeValInput = document.getElementById('kodeValidasi');
        
        if (!form || !kodeValInput) return;
        
        const kodeVal = kodeValInput.value;
        ...
        if (!nikInput || !whatsappInput) return;
        ...
        document.querySelectorAll('.fish-checkbox:checked').forEach(cb => {
            if (cb.value === "Lainnya") {
                const otherValInput = document.getElementById('jenisIkanLainnya');
                if (otherValInput) {
                    const otherVal = otherValInput.value.trim();
        ...
        const isOwner = document.getElementById('statusNelayan') ? 
            document.getElementById('statusNelayan').value === 'Pemilik Kapal' : false;
        ...
        const formData = {
            ...
            nik: nik,
            whatsapp: whatsapp,
            profesi: document.getElementById('profesi') ? document.getElementById('profesi').value : '',
            status: document.getElementById('statusNelayan') ? document.getElementById('statusNelayan').value : '',
            tahunLahir: document.getElementById('tahunLahir') ? document.getElementById('tahunLahir').value : '',
            usia: document.getElementById('usia') ? document.getElementById('usia').value : '',
            kecamatan: document.getElementById('kecamatan') ? document.getElementById('kecamatan').value : '',
            desa: document.getElementById('desa') ? document.getElementById('desa').value : '',
            alatTangkap: document.getElementById('alatTangkap') ? document.getElementById('alatTangkap').value : '',
            namaKapal: isOwner && document.getElementById('namaKapal') ? document.getElementById('namaKapal').value : '-',
            jenisKapal: isOwner && document.getElementById('jenisKapal') ? document.getElementById('jenisKapal').value : '-',
            jenisIkan: selectedFish.join(", "),
            usahaSampingan: document.getElementById('usahaSampingan') ? document.getElementById('usahaSampingan').value : '',
            tanggalValidasi: document.getElementById('tanggalValidasi') ? document.getElementById('tanggalValidasi').value : '',
            validator: document.getElementById('validator') ? document.getElementById('validator').value : '',
            driveLink: document.getElementById('driveLink') ? document.getElementById('driveLink').value : '',
            kodeValidasi: kodeVal,
    ...
    function viewDetail(id) {
        ...
        const d_wa = document.getElementById('d_wa');
        const d_domisili = document.getElementById('d_domisili');
        const d_profesi = document.getElementById('d_profesi');
        const d_status = document.getElementById('d_status');
        const d_alatTangkap = document.getElementById('d_alatTangkap');
        const d_usaha = document.getElementById('d_usaha');
        const d_ikan = document.getElementById('d_ikan');
        const d_kapal_card = document.getElementById('d_kapal_card');
        const d_namaKapal = document.getElementById('d_namaKapal');
        const d_jenisKapal = document.getElementById('d_jenisKapal');
        const d_tgl_valid = document.getElementById('d_tgl_valid');
        ...
        if (d_profesi) {
            d_profesi.innerText = d.profesi;
            let badgeClass = 'bg-primary';
            if (d.profesi === 'Nelayan Penuh Waktu') badgeClass = 'badge-profesi-penuh';
            else if (d.profesi === 'Nelayan Sambilan Utama') badgeClass = 'badge-profesi-sambilan-utama';
            else if (d.profesi === 'Nelayan Sambilan Tambahan') badgeClass = 'badge-profesi-sambilan-tambahan';
            d_profesi.className = `badge ${badgeClass}`;
        }

        if (d_status) d_status.innerText = d.status;
        if (d_alatTangkap) d_alatTangkap.innerText = d.alatTangkap;
        if (d_usaha) d_usaha.innerText = d.usahaSampingan || '-';
        ...
        if (d_ikan) {
            ...
            if (d.jenisIkan) {
                d.jenisIkan.split(', ').forEach(fish => {
                    ...
                    d_ikan.innerHTML += `<span class="badge bg-light text-dark border me-1 mb-1"><i class="fas ${iconClass} me-1"></i>${fish}</span>`;
                });
            }
        }
        
        if (d.status === 'Pemilik Kapal') {
            if (d_kapal_card) d_kapal_card.style.display = 'block';
            if (d_namaKapal) d_namaKapal.innerText = d.namaKapal;
            if (d_jenisKapal) d_jenisKapal.innerText = d.jenisKapal;
        } else {
            if (d_kapal_card) d_kapal_card.style.display = 'none';
        }
        
        if (d_tgl_valid) d_tgl_valid.innerText = d.tanggalValidasi;
        if (d_validator) d_validator.innerText = d.validator;
    ...
    function toggleBulkDeleteBtn() {
        ...
        if (btn) {
            if (checked) btn.classList.remove('d-none'); 
    ...
    function editData(id) {
        ...
        // Set nilai form
        const fields = ['nama', 'nik', 'whatsapp', 'profesi', 'tahunLahir', 'usia', 'alatTangkap', 'usahaSampingan', 'tanggalValidasi', 'validator', 'driveLink', 'kodeValidasi', 'keterangan'];
        fields.forEach(key => {
        ...
        const kecSelect = document.getElementById('kecamatan');
        if (kecSelect && d.kecamatan) {
            kecSelect.value = d.kecamatan;
            kecSelect.dispatchEvent(new Event('change'));
        }
        
        const desaSelect = document.getElementById('desa');
        if (desaSelect && d.desa) {
            setTimeout(() => {
                desaSelect.value = d.desa;
            }, 100);
        }

        const statusSelect = document.getElementById('statusNelayan');
        if (statusSelect && d.status) {
        ...
        if (d.status === 'Pemilik Kapal') {
            ...
            if (namaKapal && d.namaKapal) namaKapal.value = d.namaKapal;
            if (jenisKapal && d.jenisKapal) jenisKapal.value = d.jenisKapal;
            
            if (jenisKapal && d.jenisKapal && KAPAL_INFO[d.jenisKapal]) {
                if (kapalInfo) {
    ...
    // Fungsi untuk menangani duplicate check dari warning
    function handleDuplicateCheck() {
        document.getElementById('v-pills-data-tab').click(); 
        setTimeout(() => document.getElementById('btnCekGanda').click(), 200);
    }
    ...
    // Ekspos fungsi-fungsi penting ke window agar bisa diakses dari inline event handlers
    return {
        // Fungsi utama
        init: init,
        ...
        safeGenerateIDCard: safeGenerateIDCard,
        showNotification: showNotification,
        printData: printData,
        verifyKIN: verifyKIN,
        verifyKINAndShow: verifyKINAndShow,
        showAllKIN: showAllKIN,
        resetVerifyForm: resetVerifyForm,
        setVerifyExample: setVerifyExample,
        showFishInfoModal: showFishInfoModal,
        mergeData: mergeData,
        ...
        // Data dan settings untuk akses eksternal jika diperlukan
        appData: appData,
